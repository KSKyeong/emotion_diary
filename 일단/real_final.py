...
def recognition(f_2_s):
    ...
    if(tt == True):
        ...
        cv2.imwrite('../src/'+ emotion_text +'z'+ str(next_num) +'.jpg', bgr_image)# 변수 활용 원래는 bgr_image
        f = open(emotion_text +'z'+ str(next_num)+".txt", 'w', encoding="UTF8")
        f.close()
...
if __name__ == '__main__': 
    ...
    while(True):
        ...
        key_2_use = pyautogui.confirm(text='?', title='!!', buttons=['quit', 'map' ,'one_more'])
        #key_2_use = cv2.waitKey(0) & 0xFF
        if key_2_use == 'quit': # 종료하고 싶다.
            break
        elif key_2_use == 'map': # s 입력 시 저장 
